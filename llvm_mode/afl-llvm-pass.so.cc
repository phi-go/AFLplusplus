...
class AFLCoverage : public ModulePass {
 ...
 public:
  ...
  bool shouldInstrumentBlock(BasicBlock::iterator IP);
...
bool AFLCoverage::shouldInstrumentBlock(BasicBlock::iterator IP) {
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  if ((isatty(2) && !getenv("AFL_QUIET")) || getenv("AFL_DEBUG") != NULL) {
    SAYF(cCYA "afl-llvm-auto-ijon" VERSION cRST /*" by <syssec@philipp-goerz.com>\n"*/);
  } else {
    be_quiet = 1;
  }
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) {
      ...
      IRBuilder<>          IRB(&(*IP));
      if (!myWhitelist.empty() && !shouldInstrumentBlock(IP)) continue;
      InlineAsm *IA =
        InlineAsm::get(FunctionType::get(Type::getVoidTy(C), false), "int 3", "", true,
                       /* IsAlignStack */ false, InlineAsm::AsmDialect::AD_Intel);
      CallInst *Result = IRB.CreateCall(IA, {}, "auto_ijon_annotation");
      Result->addAttribute(AttributeList::FunctionIndex,
                           Attribute::NoUnwind); 
      BB.print(errs());
    }
  }
