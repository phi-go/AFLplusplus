      ...
      LIST_FOREACH(&afl->active_annotations, annotation_t, {
        ...
        if (num_writes) {
          switch(el->type) {
            ...
            case ANN_OVERFLOW:
              {
                if (!(el->cur_best.best_values[0]
                      || el->cur_best.best_values[2])) {
                  {
                    uint64_t overflow = el->shm_addr->result.best_values[0];
                    if (overflow) {
                      el->shm_addr->result.best_values[0] = 1;
...

                  {
                    ...
                    if (overflow) {
                      el->shm_addr->result.best_values[2] = 1;
                ...
                if (!(el->cur_best.best_values[4]
                      || el->cur_best.best_values[6])) {
                  {
                    int64_t overflow = el->shm_addr->result.best_values[4];
                    if (overflow) {
                      el->shm_addr->result.best_values[4] = 1;
...

                  {
                    ...
                    if (overflow) {
                      el->shm_addr->result.best_values[6] = 1;
        ...
        if (improvement) {
...

          {
            ...
            else if (el->new_ann_queue_files > 10) {
              disable_annotation(afl, el);
              LIST_REMOVE_CURRENT_EL_IN_FOREACH();
            }
            ...
            else if ((el->type == ANN_SET && el->num_corresponding_queue_files > 1000) 
                || (el->type == ANN_META_NODE && el->num_corresponding_queue_files > 1000)) {
              el->ignored = 1;
              disable_annotation(afl, el);
              LIST_REMOVE_CURRENT_EL_IN_FOREACH();
            }
